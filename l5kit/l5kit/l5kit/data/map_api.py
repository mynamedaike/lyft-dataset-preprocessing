...
class MapAPI:
    ...
    @lru_cache(maxsize=CACHE_SIZE)
    def get_stopline_coords(self, element_id: str) -> dict:
        """
        Get XYZ coordinates in world ref system for a stopline given its id
        lru_cached for O(1) access

        Args:
            element_id (str): stopline element id

        Returns:
            dict: a dict with the polygon coordinates as an (Nx3) XYZ array
        """
        element = self[element_id]
#        assert self.is_stopline(element)
        traffic_element = element.element.traffic_control_element
